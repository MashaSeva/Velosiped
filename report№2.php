...
$clientStats = $ordersTable->getClientOrderStats($startDate, $endDate);
?>
...
<head>
    <meta charset="UTF-8">
    <title>Отчёт №2 - Средняя сумма заказов по клиентам</title>
...
<body>
    <header>
        ...
        <h1>Отчёт №2 - Средняя сумма заказов по клиентам</h1>
        <nav class="nav-links">
    ...
    <div class="report-container">
        ...
        <table>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($clientStats as $index => $stat): ?>
                    <tr>
                        <td><?= $index + 1 ?></td>
                        <td><?= htmlspecialchars($stat['client_id']) ?></td>
    ...
    <a href="export_report.php?type=client_stats&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>" 
       class="export-btn">Экспорт в Excel</a>
    <a href="export_report.php?type=client_stats&format=word&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>" 
